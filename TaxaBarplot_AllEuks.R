### PARAGON ANALYSES - TAXA BARPLOT - ALL EUKS ###
...
taxz$fin <- ifelse(taxz$o==" Dinophyceae","Dinoflagellates",NA)
taxz$fin <- ifelse(taxz$c==" Choanoflagellida","Choanoflagellates",taxz$fin)
taxz$fin <- ifelse(grepl("MAST",taxz$c),"MAST cells",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Foraminifera","Foraminifera",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Haptophyta","Haptophytes",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Chlorophyta","Chlorophytes",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Cryptophyta","Cryptophytes",taxz$fin)
taxz$fin <- ifelse(taxz$o==" Ascomycota","Fungi",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Ascomycota","Fungi",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Fungi","Fungi",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Cercozoa","Cercozoans",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Ciliophora","Ciliates",taxz$fin)
taxz$fin <- ifelse(taxz$c==" Discoba","Discobids",taxz$fin)
taxz$fin <- ifelse(taxz$p==" Amoebozoa","Amoebozoans",taxz$fin)
taxz$fin <- ifelse(taxz$o==" Bacillariophyta","Diatoms",taxz$fin)
taxz$fin <- ifelse(taxz$o==" Pelagophyceae","Pelagophytes",taxz$fin)
taxz$fin <- ifelse(is.na(taxz$fin) & taxz$p==" Stramenopiles","Other Stramenopiles",taxz$fin)
taxz$fin <- ifelse(is.na(taxz$fin) & taxz$p==" Alveolata","Other Alveolates",taxz$fin)
taxz$fin <- ifelse(is.na(taxz$fin) & taxz$p==" Archaeplastida","Other Archaeplastida",taxz$fin)
...
colrs <- c("Choanoflagellates"="#D46854", "Foraminifera"="#BFA88C","Haptophytes"="#993EE8","Other Stramenopiles"="#D895DD" ,"Chlorophytes"="#72DBBD","Cryptophytes"="#E6B15C" ,"Amoebozoans"="#DBE7D4","Dinoflagellates"="#D2E04E","Discobids"="#72E16B","Diatoms"="#D2608C","Other Alveolates"="#DAB8CE","Other Eukaryotes"="#7D98D5","Cercozoans"="#C6E198","Fungi"="#84C3D7","Ciliates"="#8169D7","Pelagophytes"="#DB57D1","Other Archaeplastida"="dodgerblue","MAST cells"="khaki1")
...
ggsave("TaxaBarplot_AllEuks.pdf",width=13,height=6)
